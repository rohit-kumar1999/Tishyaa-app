...
const calculateTotals = (items: CartItem[], discount: number = 0, tax: number = 0, shipping: number = 0) => {
...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  ...
  switch (action.type) {
    ...
    case 'LOAD_CART': {
      const { subtotal, total, itemCount } = calculateTotals(action.payload, state.discount, state.tax, state.shipping);
      return {
        ...state,
    ...
    case 'ADD_ITEM': {
      ...
      const { subtotal, total, itemCount } = calculateTotals(newItems, state.discount, state.tax, state.shipping);
      return {
        ...state,
        items: newItems,
        subtotal,
    ...
    case 'APPLY_DISCOUNT': {
      const { subtotal, total, itemCount } = calculateTotals(state.items, action.payload, state.tax, state.shipping);
      return {
        ...state,
    ...
    case 'SET_SHIPPING': {
      const { subtotal, total, itemCount } = calculateTotals(state.items, state.discount, state.tax, action.payload);
      return {
        ...state,
    ...
    case 'SET_TAX': {
      const { subtotal, total, itemCount } = calculateTotals(state.items, state.discount, action.payload, state.shipping);
      return {
        ...state,
        tax: action.payload,
        subtotal,
      ...
      try {
        ...
        if (savedCart) {
          ...
          if (savedMetadata) {
            ...
            if (metadata.tax) dispatch({ type: 'SET_TAX', payload: metadata.tax });
            if (metadata.shipping) dispatch({ type: 'SET_SHIPPING', payload: metadata.shipping });
            if (metadata.discount) dispatch({ type: 'APPLY_DISCOUNT', payload: metadata.discount });
          }
        } else {
      ...
      const saveCart = async () => {
        try {
          await Promise.all([
            ...
            AsyncStorage.setItem(CART_METADATA_KEY, JSON.stringify({
              tax: state.tax,
