...
const createOrderService = (data: CreateOrderRequest): Promise<CreateOrderResponse> => {
...
const verifyPaymentService = (data: any): Promise<PaymentVerificationResponse> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        success: true,
...
export const usePayment = () => {
  const { userId, userFullName, primaryEmailAddress } = useUserData()
  const [isProcessing, setIsProcessing] = useState(false)
  const [paymentStep, setPaymentStep] = useState<'idle' | 'creating' | 'processing' | 'verifying'>('idle')

  // API mutations
  const createOrderMutation = useApiMutation<CreateOrderResponse, CreateOrderRequest>(
    '/orders/create',
    {
  ...
  const verifyPaymentMutation = useApiMutation<PaymentVerificationResponse, any>(
    '/payments/verify',
    {
  ...
  const processPayment = async (paymentRequest: PaymentRequest): Promise<boolean> => {
    if (!userId) {
      toast({
  ...
  const processRazorpayPayment = async (order: CreateOrderResponse): Promise<boolean> => {
    return new Promise((resolve) => {
      Alert.alert(
        'Razorpay Payment',
